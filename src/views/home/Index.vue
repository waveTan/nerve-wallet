...
<script>
  ...
  import {divisionDecimals, copys, addressInfo, Times, superLong, connectToExplorer, Plus} from '@/api/util'
        ...
        await this.$post('/', 'getAccountLedgerList', [address], 'Home')
          .then((response) => {
            ...
            if (response.hasOwnProperty("result")) {
              ...
              newAssetsList.balance = Number(divisionDecimals(response.result[0].balance)).toFixed(3);
              newAssetsList.locking = Number(divisionDecimals(Plus(response.result[0].consensusLock, response.result[0].timeLock))).toFixed(3);
              newAssetsList.total = Number(divisionDecimals(response.result[0].totalBalance)).toFixed(3);
            } else {
          ...
          .then((response) => {
            ...
            if (response.hasOwnProperty("result")) {
              ...
              for (let itme of response.result.list) {
                ...
                itme.type = 2;
                itme.total = Number(divisionDecimals(itme.balance, itme.decimals)).toFixed(3);
                itme.locking = '--';
                itme.balance = Number(divisionDecimals(itme.balance, itme.decimals)).toFixed(3);
          ...
          .then((response) => {
            ...
            if (response.hasOwnProperty("result")) {
              for (let item of response.result) {
                item.totalBalance = divisionDecimals(item.totalBalance);
                item.balance = divisionDecimals(item.balance);
                item.locking = divisionDecimals(item.consensusLock + item.timeLock);
              }
